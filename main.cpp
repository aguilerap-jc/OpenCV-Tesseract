...
int main(int /*argc*/, char** /*argv*/)
{
	...
	while(salida){
		...
		cvtColor(frame, edges,CV_BGR2RGBA);
	    //TESSERACT
		do{
	    tess.Init(NULL, "eng", OEM_DEFAULT);
   		tess.SetPageSegMode(PSM_SINGLE_BLOCK);
   		tess.SetImage((uchar*)edges.data, edges.cols, edges.rows, 1, edges.cols);
   		}while(usleep(1000));
    	...
    	// Get the text wit tesseract
	    char* out = tess.GetUTF8Text();
	    cout << out << endl;

		if(waitKey(30) >=0) salida = false;
	}
